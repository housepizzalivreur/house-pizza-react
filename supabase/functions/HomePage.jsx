...
function CheckoutView({
  ...
  const handlePaymentSubmit = async () => {
    ...
    onSuccess({
      // ⚠️ pas de `number` ici : c'est la DB qui le génère (séquence)
      total: finalTotal,
...
function ConfirmationView({ orderData, onBack, dark }) {
  ...
  // Persister la progression côté Supabase
  useEffect(() => {
    if (!orderData.id) return; // commande pas encore créée en DB
    (async () => {
      try {
        await updateOrder(orderData.id, { statusIdx });
      } catch (err) {
        console.error("SYNC ERROR:", err);
      }
    })();
  }, [statusIdx, orderData.id]);
...
function OrdersOverviewPanel({ dark }) {
  ...
  const cancelledToday = orders.filter(
    (o) => o.cancelled && inToday(o.updatedAt || o.createdAt || 0),
  );
...
function ExportPanel({ dark }) {
  ...
  const exportOrders = async () => {
    const rows = periodFilter(await DriverOrdersDB.load(), period).map((o) => ({
      "N°": o.number,
  ...
  const exportItems = async () => {
    const orders = periodFilter(await DriverOrdersDB.load(), period);
    const rows = [];
    orders.forEach((o) => {
      (o.items || []).forEach((it) =>
        rows.push({
...
/* ═══════════════════════════════════════════════════
   DRIVER ORDERS DB - wrapper async autour de Supabase
...
const DriverOrdersDB = {
  load: async () => {
    try {
      ...
      console.error("DriverOrdersDB.load:", e);
      return [];
    }
  },
  update: async (number, patch) => {
    try {
      await updateOrderByNumber(number, patch);
    } catch (e) {
      console.error("DriverOrdersDB.update:", e);
    }
  },
  subscribe: (onChange) => subscribeOrders(onChange),
};
...
function CashierView({ onBack, onNewOrder, dark, showToast }) {
  console.log("CASHIER VIEW MOUNTED");
  ...
  useEffect(() => {
    let cancelled = false;
    const load = async () => {
      const data = await DriverOrdersDB.load();
      if (!cancelled) setOrders(data);
    };
    load();
    const unsub = DriverOrdersDB.subscribe(load);
    return () => {
      cancelled = true;
      unsub();
    };
  ...
  const refresh = async () => {
    setOrders(await DriverOrdersDB.load());
    setTick((t) => t + 1);
  };
  ...
  if (selected) {
    ...
    const toggleItem = async (idx) => {
      ...
      await DriverOrdersDB.update(selected.number, patch);
      refresh();
    };

    const startPrep = async () => {
      await DriverOrdersDB.update(selected.number, { statusIdx: 1 });
      refresh();
    };

    const forceReady = async () => {
      await DriverOrdersDB.update(selected.number, { statusIdx: 2 });
      refresh();
    };
      ...
      <div className="max-w-2xl mx-auto px-4 pb-20 pt-6">
        ...
        {selected.orderType !== "delivery" && selected.statusIdx === 2 && (
          <div className="space-y-2 mb-3">
            ...
            <button
              onClick={async () => {
                await DriverOrdersDB.update(selected.number, { statusIdx: 6 });
                refresh();
                setSelectedNum(null);
...
function DriverView({ onBack, dark, showToast }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const data = await DriverOrdersDB.load();
      if (cancelled) return;
      // Applique l'overlay `arrived` depuis la Set locale
      setOrders(
        data.map((o) =>
          arrivedRef.current.has(o.number) ? { ...o, arrived: true } : o,
        ),
      );
    };
    load();
  ...
  if (selected) {
    ...
    const pickup = async () => {
      await DriverOrdersDB.update(selected.number, { statusIdx: 3 });
      refresh();
    };
    const startRoute = async () => {
      await DriverOrdersDB.update(selected.number, { statusIdx: 4 });
      refresh();
    };
    const markArrived = () => {
      arrivedRef.current.add(selected.number);
      if (selected.whatsappOptIn && selected.whatsappNumber) {
        sendWhatsApp(
          selected.whatsappNumber,
      ...
      // Re-render avec l'overlay local
      setOrders((prev) =>
        prev.map((o) =>
          o.number === selected.number ? { ...o, arrived: true } : o,
        ),
      );
    };
    const markDelivered = async () => {
      await DriverOrdersDB.update(selected.number, { statusIdx: 5 });
      arrivedRef.current.delete(selected.number);
      refresh();
      setSelectedNum(null);
...
function OrderTrackingView({ dark, onBack }) {
  ...
  // Cherche la commande dans Supabase
        ...
        <CheckoutView
          ...
          onSuccess={async (data) => {
            ...
            try {
              ...
              const ONLINE = ["card", "applepay", "googlepay"];
